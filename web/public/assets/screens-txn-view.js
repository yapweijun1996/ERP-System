...
(function canonicalSalesEnquiryWorkspace(){
  ...
  function initials(name){
    return String(name||'').trim().split(/\s+/).slice(0,2).map(word=>word[0]||'').join('').toUpperCase()||'C';
  }
  ...
  SCREENS['txn-view']=async function(root){
    ...
    root.innerHTML=`<div class="content full"><section class="master" data-sales-transaction="canonical" data-record-id="${Number(enquiry.id)}" data-related-count="${data.quotations.length}"><div class="scrollarea">
      ...
      <div class="docwrap"><div class="docpage"><div class="dochead"><div class="dh-row1"><div><div class="dt">${ic('comment')}${esc(s('title'))} <span class="dnum">${esc(enquiry.docNo)}</span></div><div class="h1sub">${esc(enquiry.subject)}</div></div><div class="dactions">${cap(statusLabel(s,enquiry.status),statusTone(enquiry.status))}</div></div>
        <div class="docmeta"><div class="dm"><small>${esc(s('customer'))}</small><div class="partner"><span class="pav">${esc(initials(customer.name))}</span><b>${esc(customer.name||'#'+enquiry.customerId)}</b></div></div><div class="dm"><small>${esc(s('date'))}</small><b>${esc(dateValue(enquiry.enquiryDate))}</b></div><div class="dm"><small>${esc(s('channel'))}</small><b>${esc(enquiry.channel)}</b></div><div class="dm"><small>${esc(s('owner'))}</small><b>${esc(enquiry.ownerName)}</b></div><div class="dm"><small>${esc(s('status'))}</small><b>${esc(statusLabel(s,enquiry.status))}</b></div></div></div>
