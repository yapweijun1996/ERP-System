...
SCREENS['opportunity'] = async function(root,params){
  ...
  function render(){
    ...
    root.innerHTML=`<div class="content full"><section class="master"><div class="docwrap"><div class="docpage">
      ...
      <div class="dochead"><div class="dh-row1"><div><h1 class="dt">${ic('handshake')}${esc(d.title)} <span class="dnum">${esc(d.docNo)}</span></h1><div style="color:var(--muted);font-size:13px;margin-top:4px">${esc(c.name)} · ${esc(s('owner'))} ${esc((DB.user&&DB.user.name)||'—')}</div></div><div class="dactions">${cap(crmStageLabel(stage),crmStageTone(stage.charAt(0).toUpperCase()+stage.slice(1)))}${btn(s('customer360'),{icon:'user',cls:'soft',attrs:'data-customer="1"'})}</div></div>
...
function customer360Copy(){
  ...
  const packs={
    ...
    ms:{
      ...
      editCustomer:'Edit pelanggan',editCustomerTitle:'Edit data induk pelanggan',customerCode:'Kod pelanggan',customerName:'Nama pelanggan',industry:'Industri',saveChanges:'Simpan perubahan',customerUpdated:'Pelanggan {name} dikemas kini',customerUpdateError:'Pelanggan tidak dapat disimpan',
    ...
    zh:{
      ...
      editCustomer:'编辑客户',editCustomerTitle:'编辑客户主档',customerCode:'客户编号',customerName:'客户名称',industry:'行业',saveChanges:'保存更改',customerUpdated:'客户 {name} 已更新',customerUpdateError:'无法保存客户',
    ...
    ja:{
      ...
      editCustomer:'顧客を編集',editCustomerTitle:'顧客マスターを編集',customerCode:'顧客コード',customerName:'顧客名',industry:'業種',saveChanges:'変更を保存',customerUpdated:'顧客 {name} を更新しました',customerUpdateError:'顧客を保存できませんでした',
  ...
  "editCustomerTitle": "Sửa dữ liệu chính khách hàng",
  "customerCode": "Mã khách hàng",
  "customerName": "Tên khách hàng",
  "industry": "Ngành nghề",
  "saveChanges": "Lưu thay đổi",
  "customerUpdated": "Đã cập nhật khách hàng {name}",
  "customerUpdateError": "Không thể lưu khách hàng",
  "nameLabel": "Tên",
  "roleLabel": "Vai trò",
  "emailLabel": "E-mail",
