...
SCREENS['sys-settings']=async function(root){
  ...
  const seqRows=(data.sequences||[]).map(row=>`<tr data-sequence="${row.id}"><td class="l li-name"><b>${esc(row.documentType)}</b><small>${esc(row.prefix)}${String(row.nextNumber).padStart(row.padding,'0')}</small></td><td class="l mono">${esc(row.prefix)}</td><td class="tnum">${num(row.nextNumber)}</td><td class="tnum">${num(row.padding)}</td><td class="l">${esc(row.resetPolicy)}</td><td class="r"><button class="btn plain sm" data-seq-edit>${ic('edit')}<span>${esc(c('edit'))}</span></button></td></tr>`).join('');
