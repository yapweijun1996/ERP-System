...
function controlPlaneCopy(){
  ...
  const summaryCopy={
    en:{tenantCompanies:'Companies',activeCompanyUsers:'Active company users',tenantRoles:'Roles',companyUsers:'Company users',tenantScope:'Current tenant',companyScope:'Current company'},
    zh:{tenantCompanies:'公司',activeCompanyUsers:'已启用用户',tenantRoles:'角色',companyUsers:'当前公司用户',tenantScope:'当前租户',companyScope:'当前公司'},
    ms:{tenantCompanies:'Syarikat',activeCompanyUsers:'Pengguna syarikat aktif',tenantRoles:'Peranan',companyUsers:'Pengguna syarikat',tenantScope:'Penyewa semasa',companyScope:'Syarikat semasa'},
    ja:{tenantCompanies:'会社',activeCompanyUsers:'有効なユーザー',tenantRoles:'ロール',companyUsers:'現在の会社のユーザー',tenantScope:'現在のテナント',companyScope:'現在の会社'},
    vi:{tenantCompanies:'Công ty',activeCompanyUsers:'Người dùng đang hoạt động',tenantRoles:'Vai trò',companyUsers:'Người dùng công ty',tenantScope:'Tenant hiện tại',companyScope:'Công ty hiện tại'},
  };
  const p=i18nLegacy(packs);
  return key=>summaryCopy[getLang()]?.[key]||p[key]||packs.en[key]||key;
}
...
SCREENS['master-control']=async function(root){
  ...
  const body=`<div data-canonical-master-control="true"><div class="statwrap"><div class="statcards master-control-summary">${[[c('tenantCompanies'),data.summary.tenantCompanies,c('tenantScope')],[c('activeCompanyUsers'),data.summary.activeCompanyUsers,c('companyScope')],[c('tenantRoles'),data.summary.tenantRoles,c('tenantScope')]].map(([label,value,scope])=>`<div class="card"><div><small>${esc(label)}</small><span class="summary-scope">${esc(scope)}</span></div><b class="tnum">${num(value)}</b></div>`).join('')}</div></div><div class="toolbar"><div class="master-control-identity"><b>${esc(data.master.name)}</b><span class="mono">${esc(data.master.masterFn)}</span></div><div class="grow"></div><div class="master-control-actions">${btn(c('manageUsers'),{icon:'people',cls:'soft',attrs:'onclick="navigate(\'user-mgmt\')"'})}${btn(c('manageRoles'),{icon:'shield',cls:'soft',attrs:'onclick="navigate(\'role-permission\')"'})}</div></div><div class="panel" style="margin:0 24px 14px"><div class="panel-h"><h3>${esc(c('companies'))}</h3></div><div class="tablewrap"><table class="lines"><thead><tr><th class="l">${esc(c('company'))}</th><th class="l">${esc(c('country'))}</th><th class="l">${esc(c('currency'))}</th><th class="l">${esc(c('tax'))}</th><th>${esc(c('userCount'))}</th><th class="l"></th></tr></thead><tbody>${companyRows}</tbody></table></div></div><div class="panel" style="margin:0 24px 24px"><div class="panel-h"><h3>${esc(c('companyUsers'))}</h3></div><div class="tablewrap"><table class="lines"><thead><tr><th class="l">${esc(c('user'))}</th><th class="l">${esc(c('role'))}</th><th class="l">${esc(c('access'))}</th><th class="l">${esc(c('status'))}</th></tr></thead><tbody>${userRows}</tbody></table></div></div></div>`;
  root.innerHTML=modulePage({module:'admin',route:'master-control',active:'master-control',title:c('tenant'),sub:c('tenantSub'),action:btn(c('settings'),{icon:'gear',cls:'primary',attrs:'onclick="navigate(\'sys-settings\')"'}),body});
};
...
SCREENS['sys-settings']=async function(root){
  ...
  const factsPanel=`<div class="panel" style="margin:0 24px 14px">
    <div class="panel-h"><h2 id="systemSettingsFactsTitle">${esc(c('companyFacts'))}</h2></div>
    <dl class="system-settings-facts" aria-labelledby="systemSettingsFactsTitle">
      <div class="system-settings-fact"><dt>${esc(c('company'))}</dt><dd>${esc(data.company.name)}</dd></div>
      <div class="system-settings-fact"><dt>${esc(c('country'))}</dt><dd>${esc(data.company.country)}</dd></div>
      <div class="system-settings-fact"><dt>${esc(c('currency'))}</dt><dd>${esc(data.company.currency)}</dd></div>
      <div class="system-settings-fact"><dt>${esc(c('tax'))}</dt><dd>${esc(data.company.taxRegime)}</dd></div>
    </dl>
  </div>`;
  const policyPanel=`<div class="panel" style="margin:0 24px 14px">
    <div class="panel-h"><h2>${esc(c('general'))}</h2></div>
    <div class="panel-body"><div class="system-settings-policy-grid">
      <label class="fld"><span>${esc(c('dateFormat'))}</span><select id="cpDateFormat">${['YYYY-MM-DD','DD/MM/YYYY','MM/DD/YYYY'].map(v=>`<option ${v===policy.dateFormat?'selected':''}>${v}</option>`).join('')}</select></label>
      <label class="fld"><span>${esc(c('negativeStock'))}</span><select id="cpStockPolicy"><option value="block" ${policy.negativeStockPolicy==='block'?'selected':''}>${esc(c('block'))}</option><option value="warn" ${policy.negativeStockPolicy==='warn'?'selected':''}>${esc(c('warn'))}</option></select></label>
      <label class="fld"><span>${esc(c('threshold'))}</span><input id="cpThreshold" inputmode="decimal" value="${esc(policy.approvalThreshold)}"></label>
      <label class="fld"><span>${esc(c('timeout'))}</span><input id="cpTimeout" type="number" min="15" max="1440" value="${num(policy.sessionTimeoutMinutes)}"></label>
      <label class="fld"><span>${esc(c('warehouse'))}</span><input id="cpWarehouse" value="${esc(policy.defaultWarehouseCode||'')}"></label>
    </div></div>
    <div class="system-settings-policy-actions">${btn(c('saveSettings'),{icon:'check',cls:'primary',attrs:'data-policy-save'})}</div>
  </div>`;
  const body=`<div data-canonical-system-settings="true" class="system-settings-layout">${factsPanel}${policyPanel}<div class="panel" style="margin:0 24px 14px"><div class="panel-h"><h2>${esc(c('sequences'))}</h2></div><div class="tablewrap"><table class="lines"><thead><tr><th class="l">${esc(c('document'))}</th><th class="l">${esc(c('prefix'))}</th><th>${esc(c('next'))}</th><th>${esc(c('padding'))}</th><th class="l">${esc(c('reset'))}</th><th></th></tr></thead><tbody>${seqRows||`<tr><td colspan="6">${esc(c('noData'))}</td></tr>`}</tbody></table></div></div><div class="panel" style="margin:0 24px 14px"><div class="panel-h"><h2>${esc(c('periods'))}</h2></div><div class="tablewrap"><table class="lines"><thead><tr><th class="l">${esc(c('period'))}</th><th class="l">${esc(c('dates'))}</th><th class="l">${esc(c('status'))}</th><th></th></tr></thead><tbody>${periodRows||`<tr><td colspan="4">${esc(c('noData'))}</td></tr>`}</tbody></table></div></div><div class="panel" style="margin:0 24px 24px"><div class="panel-h"><h2>${esc(c('taxRules'))}</h2></div><div class="tablewrap"><table class="lines"><thead><tr><th class="l">${esc(c('code'))}</th><th>${esc(c('rate'))}</th><th class="l">${esc(c('validFrom'))}</th><th class="l">${esc(c('validTo'))}</th></tr></thead><tbody>${taxRows||`<tr><td colspan="4">${esc(c('noData'))}</td></tr>`}</tbody></table></div></div></div>`;
