...
(function erpSystemDataAdapter(){
  ...
  async function ensureSchemaUpToDate(db){
    ...
    if (!appliedDocumentTables || Number(appliedDocumentTables.n) !== 4
      ...
      || !appliedDocumentProcessing || Number(appliedDocumentProcessing.n) !== 6) {
      throw new Error(
        'Persistent PGlite migration v' + DEMO_SCHEMA_VERSION +
  ...
  var my={
    ...
    reimbursementPaymentEvidence:async function(){
      var data=await requireDemoDb().transaction(async function(tx){
        var orm=state.runtime.createOrm(tx);
        var allowed=await state.runtime.commands.hasPermissionWithin(
          orm,SCOPE,myActorUserId(),'expenses.payment.export');
        if(!allowed) throw new Error('Reimbursement bank export permission is required.');
        return state.runtime.commands.listReimbursementPaymentEvidenceWithin(orm,SCOPE);
      });
      return {data:data,meta:{
        artifactContent:'encrypted_omitted',resultEvidence:'immutable',limit:500,
      }};
    },
    configureReimbursementBankTemplate:async function(payload){
      var data=await requireDemoDb().transaction(async function(tx){
        var orm=state.runtime.createOrm(tx);
        var allowed=await state.runtime.commands.hasPermissionWithin(
          orm,SCOPE,myActorUserId(),'expenses.payment.template.manage');
        if(!allowed) throw new Error('Bank template permission is required.');
        return state.runtime.commands.configureReimbursementBankTemplateWithin(
          orm,SCOPE,myActorUserId(),payload||{});
      });
      return {data:data,meta:{versioned:true,status:'confirmed'}};
    },
    generateReimbursementBankExport:async function(payload){
      var data=await requireDemoDb().transaction(async function(tx){
        var orm=state.runtime.createOrm(tx);
        var allowed=await state.runtime.commands.hasPermissionWithin(
          orm,SCOPE,myActorUserId(),'expenses.payment.export');
        if(!allowed) throw new Error('Reimbursement bank export permission is required.');
        return state.runtime.commands.generateReimbursementBankExportWithin(
          orm,SCOPE,myActorUserId(),payload||{},{
            encrypt:encryptDemoCredential,
            decrypt:decryptDemoCredential,
            hash:function(value){return state.runtime.sha256Hex(value);},
          });
      });
      return {data:data,meta:{artifactContent:'encrypted_omitted',integrity:'sha256'}};
    },
    downloadReimbursementBankExport:async function(exportId,accessKey,purpose){
      var data=await requireDemoDb().transaction(async function(tx){
        var orm=state.runtime.createOrm(tx);
        var allowed=await state.runtime.commands.hasPermissionWithin(
          orm,SCOPE,myActorUserId(),'expenses.payment.artifact.download');
        if(!allowed) throw new Error('Bank artifact download permission is required.');
        return state.runtime.commands.accessReimbursementBankExportWithin(
          orm,SCOPE,myActorUserId(),Number(exportId),String(accessKey||''),
          String(purpose||''),{
            encrypt:encryptDemoCredential,
            decrypt:decryptDemoCredential,
            hash:function(value){return state.runtime.sha256Hex(value);},
      ...
      return {data:data,meta:{sensitiveAccess:'audited',cacheControl:'no-store'}};
    },
    importReimbursementBankResults:async function(payload){
      var data=await requireDemoDb().transaction(async function(tx){
        var orm=state.runtime.createOrm(tx);
        var allowed=await state.runtime.commands.hasPermissionWithin(
          orm,SCOPE,myActorUserId(),'expenses.payment.result.import');
        if(!allowed) throw new Error('Bank result import permission is required.');
        return state.runtime.commands.importReimbursementBankResultsWithin(
          orm,SCOPE,myActorUserId(),payload||{});
      });
    ...
    receipts:async function(){
      ...
      var result=await requireDemoDb().query(
        `select d.id,d.document_key,d.original_file_name,d.current_version_no,
                ...
                i.status as inbox_status,i.review_reasons,i.duplicate_of_version_id,
                i.submission_kind,i.submitted_at,
                a.auto_submit_authorized
         ...
         left join receipt_inbox_item i
           on i.master_fn=v.master_fn and i.company_fn=v.company_fn and i.version_id=v.id
         left join receipt_upload_authorization a
           on a.master_fn=v.master_fn and a.company_fn=v.company_fn and a.version_id=v.id
      ...
      return {data:result.rows.map(function(row){return {
        ...
        inboxStatus:row.inbox_status,reviewReasons:row.review_reasons||[],
        duplicateOfVersionId:row.duplicate_of_version_id==null?null:Number(row.duplicate_of_version_id),
        submissionKind:row.submission_kind,submittedAt:row.submitted_at,
        autoSubmitAuthorized:Boolean(row.auto_submit_authorized),
      };}),meta:{actorDerived:true,availability:'capture',scanning:'fail_closed',
        receiptInbox:'confidence_governed',limit:100}};
    },
    uploadReceipt:async function(draft){
      ...
      var result=await requireDemoDb().transaction(async function(tx){
        ...
        if(existing.rows[0]){
          ...
          await tx.query(
            `insert into receipt_upload_authorization
               (master_fn,company_fn,version_id,uploader_user_id,auto_submit_authorized,
                authorized_at,statement_version)
             values ($1,$2,$3,$4,$5,$6,'receipt-auto-submit-v1')
             on conflict (master_fn,company_fn,version_id) do nothing`,
            [SCOPE.masterFn,SCOPE.companyFn,row.version_id,myActorUserId(),
             Boolean(draft.autoSubmitAuthorized),
             draft.autoSubmitAuthorized?new Date():null]);
          var existingAuthorization=await tx.query(
            `select auto_submit_authorized from receipt_upload_authorization
             where master_fn=$1 and company_fn=$2 and version_id=$3 limit 1`,
          ...
          return {row:row,replayed:true,
            autoSubmitAuthorized:Boolean(existingAuthorization.rows[0].auto_submit_authorized)};
        }
        ...
        await tx.query(
          `insert into receipt_upload_authorization
           ...
           values ($1,$2,$3,$4,$5,$6,'receipt-auto-submit-v1')
           on conflict (master_fn,company_fn,version_id) do nothing`,
          [SCOPE.masterFn,SCOPE.companyFn,version.rows[0].id,myActorUserId(),
           Boolean(draft.autoSubmitAuthorized),
           draft.autoSubmitAuthorized?new Date():null]);
        await tx.query(
          `insert into document_scan_job
        ...
        return {row:Object.assign({},documentRow,version.rows[0]),replayed:false,
          autoSubmitAuthorized:Boolean(draft.autoSubmitAuthorized)};
      });
      return {data:{
