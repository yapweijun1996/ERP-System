...
(function canonicalSalesOrderApprovalScreens(){
  ...
  async function loadAuthoringData(){
    const pages=await Promise.all([
      adapter().list('sales/customers',{limit:100}),
      adapter().list('inventory/products',{limit:100}),
    ]);
    return {customers:pages[0].data||[],products:pages[1].data||[]};
  }

  SCREENS['new-sales-order']=async function(root){
    const s=copy(),data=await loadAuthoringData();
    const state={lines:data.products.length?[{productId:Number(data.products[0].id),qty:'1',unitPrice:'0.00',taxCode:taxCode()}]:[]};
    function productOptions(selected){return data.products.map(row=>`<option value="${row.id}" ${Number(selected)===Number(row.id)?'selected':''}>${esc(row.sku)} · ${esc(row.name)}</option>`).join('');}
    function renderLines(){
      const body=root.querySelector('[data-so-lines]');if(!body)return;
      body.innerHTML=state.lines.length?state.lines.map((line,index)=>`<tr data-line="${index}"><td class="lineno">${index+1}</td><td class="l"><select class="lineinput" data-product>${productOptions(line.productId)}</select></td><td><input class="lineinput" data-qty type="number" min="0.0001" step="0.0001" value="${esc(line.qty)}"></td><td><input class="lineinput" data-price type="number" min="0" step="0.0001" value="${esc(line.unitPrice)}"></td><td class="l"><input class="lineinput" data-tax maxlength="12" value="${esc(line.taxCode)}"></td><td class="tnum"><b data-line-net>${amount(lineNet(line))}</b></td><td style="text-align:center"><button class="iconbtn" data-remove data-tip="${esc(s('remove'))}">${ic('trash')}</button></td></tr>`).join(''):`<tr><td colspan="7" style="text-align:center;color:var(--muted);padding:24px">${esc(s('emptyProducts'))}</td></tr>`;
      body.querySelectorAll('tr[data-line]').forEach(row=>{
        const index=Number(row.dataset.line),line=state.lines[index];
        row.querySelector('[data-product]').addEventListener('change',event=>{line.productId=Number(event.target.value);});
    ...
    root.innerHTML=`<div class="content full"><section class="master" data-canonical-sales-order-authoring="true"><div class="scrollarea"><div class="pagehead">${crumbs([DB.company.name,{label:t('nav.sales'),route:'sales-home'},{label:'Sales Orders',route:'sales-orders'},{cur:s('newOrder')}])}${salesNav('sales-orders')}<div class="h1row" style="margin-top:13px"><h1>${esc(s('newOrder'))}</h1>${cap(s('pending'),'warn')}</div><div class="h1sub">${esc(s('newSub'))}</div></div>
      ...
      ${data.customers.length&&data.products.length?`<div class="responsive-actionbar">${btn(s('back'),{icon:'chevleft',cls:'soft',attrs:'data-so-back'})}<div class="grow"></div>${btn(s('submit'),{icon:'flow',cls:'primary',sm:false,attrs:'data-submit-so'})}</div>`:''}</section></div>`;
    if(!data.customers.length||!data.products.length)return;
    renderLines();
    root.querySelector('[data-add-so-line]').addEventListener('click',()=>{state.lines.push({productId:Number(data.products[0].id),qty:'1',unitPrice:'0.00',taxCode:taxCode()});renderLines();});
    ...
    root.querySelector('[data-submit-so]').addEventListener('click',async event=>{
      ...
      if(!state.lines.length){toast(s('emptyProducts'),'warn');return;}
      const docNo=root.querySelector('#canonicalSoNo').value.trim();
      const reason=root.querySelector('#canonicalSoReason').value.trim();
      if(!docNo||!reason||state.lines.some(line=>Number(line.qty)<=0||Number(line.unitPrice)<0||!line.taxCode)){toast(s('taxHelp'),'warn');return;}
      button.disabled=true;button.textContent=s('creating');
      try{
        await adapter().create('sales/orders',{docNo,customerId:Number(root.querySelector('#canonicalSoCustomer').value),orderDate:root.querySelector('#canonicalSoDate').value,currency:root.querySelector('#canonicalSoCurrency').value.trim().toUpperCase(),approvalReason:reason,lines:state.lines.map(line=>({productId:Number(line.productId),qty:String(line.qty),unitPrice:String(line.unitPrice),taxCode:line.taxCode}))});
  ...
  async function loadApprovals(){
    const pages=await Promise.all([
      adapter().list('sales/order-approvals',{limit:100}),adapter().list('sales/orders',{limit:100}),adapter().list('sales/order-lines',{limit:100}),adapter().list('sales/customers',{limit:100}),adapter().list('inventory/products',{limit:100}),
    ]);
    const approvals=pages[0].data||[],orders=pages[1].data||[],lines=pages[2].data||[],customers=pages[3].data||[],products=pages[4].data||[];
  ...
  function openReview(request){
    const s=copy();
    const lineTable=buildTable({rowId:line=>line.id,columns:[{label:s('product'),render:line=>`<div class="cellsub"><b>${esc(line.product.name||'#'+line.productId)}</b><small>${esc(line.product.sku||'')}</small></div>`},{label:s('qty'),align:'r',render:line=>`<span class="tnum">${num(Number(line.qty))}</span>`},{label:s('unitPrice'),align:'r',render:line=>`<span class="tnum">${amount(line.unitPrice,request.order.currency)}</span>`},{label:s('taxCode'),render:line=>esc(line.taxCode)},{label:s('net'),align:'r',render:line=>`<b class="tnum">${amount(line.netAmount,request.order.currency)}</b>`}],rows:request.lines});
