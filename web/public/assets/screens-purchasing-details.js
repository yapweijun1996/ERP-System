...
SCREENS['goods-receipt']=async function(root,params){
  const s=purchasingDetailCopy();
  const requestedId=params&&params.receiptId?Number(params.receiptId):null;
  let detail;
  try{
    detail=await prepareGoodsReceiptDetail(requestedId);
  }catch(error){
    postingDetailPage(root,{
      module:'purchasing',route:'goods-receipt',active:'goods-receipts',
      title:s('receipt'),description:s('receiptSub'),
      crumb:[DB.company.name,{label:s('purchasing'),route:'purchasing-home'},{label:s('receipts'),route:'goods-receipts'},{cur:s('receipt')}],
      error:{title:s('receiptError'),description:error&&error.message||s('receiptError'),retryLabel:s('retry'),onRetry:()=>navigate('goods-receipt',params)},
      empty:{icon:'receive',title:s('receiptError'),description:s('receiptSub')},
      afterRender:({postingRoot})=>{
        postingRoot?.setAttribute('data-canonical-goods-receipt','true');
      },
    });
    return;
  }
  const {receipt,order,lines,movements}=detail;
  const crumb=[DB.company.name,{label:s('purchasing'),route:'purchasing-home'},{label:s('receipts'),route:'goods-receipts'}];
  if(!receipt){
    postingDetailPage(root,{
      module:'purchasing',route:'goods-receipt',active:'goods-receipts',
      title:s('receipt'),description:s('receiptSub'),
      crumb:[...crumb,{cur:s('receipt')}],
      empty:{
        icon:'receive',
        title:requestedId?s('missingReceipt'):s('noReceipt'),
        description:s('receiptSub'),
      },
      afterRender:({postingRoot})=>{
        postingRoot?.setAttribute('data-canonical-goods-receipt','true');
      },
  ...
  const currency=order&&order.currency||DB.company.currency;
  const receivedQty=lines.reduce((sum,row)=>sum+purchasingNumber(row.qty),0);
  ...
  ],rows:movements}):`<div class="posting-inline-empty">${ic('history')}<span>${esc(s('noMovement'))}</span></div>`;
  const main=`<section class="posting-detail-card" data-posting-lines data-goods-receipt-lines>
      <div class="posting-detail-card-head"><h3>${esc(s('lines'))}</h3><span class="grow"></span><small class="tnum">${lines.length} ${esc(s('lineCount'))}</small></div>
      <div class="posting-lines-scroll">${purchasingDetailLines(lines,currency,true)}</div>
      <div class="posting-lines-footer" data-posting-totals>
        <small>${esc(s('receivedTotal'))}</small>
        <span class="tnum">${num(receivedQty)}</span>
        ${cap(s('posted'),'teal')}
      </div>
    </section>
    <section class="posting-detail-card" data-posting-audit data-goods-receipt-trace>
      <div class="posting-detail-card-head"><h3>${esc(s('inventoryTrace'))}</h3><span class="grow"></span><small class="tnum">${movements.length} ${esc(s('traceCount'))}</small></div>
      <div class="posting-lines-scroll">${movementRows}</div>
    </section>`;
  const context={body:`<section class="posting-context-card" data-posting-balance data-goods-receipt-effect>
      <small>${esc(s('stockEffect'))}</small>
      <div class="posting-balance-row"><span>${esc(s('lines'))}</span><b class="tnum">${lines.length}</b></div>
      <div class="posting-balance-row"><span>${esc(s('movementCount'))}</span><b class="tnum">${movements.length}</b></div>
      <div class="posting-balance-row total"><span>${esc(s('receivedTotal'))}</span><b class="tnum">${num(receivedQty)}</b></div>
      ${indicator({tone:'ok',icon:'checkc',label:s('posted'),value:receipt.date,sub:s('receiptNote')})}
    </section>
    <section class="posting-context-card" data-goods-receipt-immutability>
      <small>${esc(s('status'))}</small>
      <div class="posting-context-note">${ic('lock')}<p>${esc(s('immutableReceipt'))}</p></div>
    </section>`};
  const postingRoot=postingDetailPage(root,{
    module:'purchasing',route:'goods-receipt',active:'goods-receipts',
    title:s('receipt'),description:s('receiptSub'),
    crumb:[...crumb,{cur:receipt.no}],
    identity:{title:s('receipt'),code:receipt.no,meta:`${receipt.supplier} · ${receipt.po}`},
    status:{label:s('posted'),tone:'teal'},
    facts:[
      {label:s('supplier'),value:receipt.supplier},
      {label:s('po'),value:receipt.po},
      {label:s('date'),value:receipt.date},
      {label:s('warehouse'),value:receipt.warehouse},
    ],
    main,context,
    headerAction:{label:s('viewStock'),icon:'history',cls:'primary',onClick:()=>navigate('stock-movement')},
  });
  postingRoot?.setAttribute('data-canonical-goods-receipt','true');
  postingRoot?.setAttribute('data-purchasing-detail','goods-receipt');
  postingRoot?.setAttribute('data-doc-no',receipt.no);
  postingRoot?.setAttribute('data-trace-count',String(movements.length));
};
...
SCREENS['supplier-invoice']=async function(root,params){
  ...
  root.innerHTML=`<div class="content full"><section class="master" data-purchasing-detail="supplier-invoice" data-doc-no="${esc(invoice.no)}" data-trace-count="${entries.length}" data-journal-balanced="${Math.abs(debit-credit)<0.001&&debit>0?'true':'false'}"><div class="scrollarea"><div class="docwrap"><div class="docpage">
    ...
    <div class="dochead"><div class="dh-row1"><div><h1 class="dt">${ic('receipt')} ${esc(s('invoice'))} <span class="dnum">${esc(invoice.no)}</span></h1><div style="color:var(--muted);font-size:13px;margin-top:4px">${esc(invoice.supplier)} · ${esc(invoice.po)}${receipt?` · ${esc(receipt.no)}`:''}</div></div><div class="dactions">${cap(purchasingDetailStatus(invoice.rawStatus),purchasingDetailTone(invoice.rawStatus))}</div></div>
