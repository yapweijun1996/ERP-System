...
(function(){
  const COPY={
    ...
    ms:{
  "bi": "Pelaporan / BI",
  "dashboard": "Papan pemuka pengurusan",
  "dashboardSub": "Paparan pengurusan terhad yang dibina semula daripada fakta komersial, pembelian, inventori dan lejar kanonik.",
  "sales": "Analisis jualan",
  "salesSub": "Hasil invois mengikut kategori produk selepas kredit produk yang boleh dijejak.",
  "stock": "Penuaan aktiviti stok",
  "stockSub": "Stok bernilai semasa mengikut hari sejak pergerakan masuk terkini direkodkan.",
  "recognized": "Hasil diiktiraf",
  "receivables": "Belum terima terbuka",
  "inventory": "Nilai inventori",
  "cash": "Baki tunai / bank",
  "openSales": "Pesanan jualan terbuka",
  "openPurchases": "Pesanan belian terbuka",
  "monthly": "Hasil bulanan diiktiraf",
  "customers": "Pelanggan utama",
  "category": "Jualan mengikut kategori produk",
  "aging": "Kedudukan penuaan stok",
  "open": "Buka laporan",
  "invoiced": "Hasil diinvois",
  "credits": "Kredit produk",
  "productRevenue": "Hasil berpunca produk",
  "units": "Unit bersih",
  "invoices": "Invois",
  "share": "Bahagian",
  "item": "item",
  "warehouse": "Gudang",
  "lastInbound": "Masuk terkini",
  "age": "Umur",
  "bucket": "Kumpulan",
  "qty": "Stok semasa",
  "value": "Nilai kos",
  ...
  "older": "61–90 hari",
  "slow": "Lebih 90 hari",
  "noHistory": "Tiada jejak masuk",
  "days": "hari",
  "source": "Sumber kanonik",
  "sourceBody": "Jumlah disegar daripada fakta kanonik semasa. Tiada nombor papan pemuka bebas disimpan.",
  ...
  "bounded": "Laporan interaktif menunjukkan paling banyak 100 kedudukan stok bernilai tertinggi. Nilai inventori syarikat merangkumi semua kedudukan.",
  "displayed": "Nilai dipaparkan",
  "positions": "Kedudukan stok",
  "slowValue": "Nilai lebih 90 hari",
  "empty": "Belum ada fakta pelaporan kanonik yang sepadan.",
  "loading": "Memuatkan fakta pelaporan kanonik…",
  "error": "Fakta pelaporan tidak dapat dimuatkan.",
  "retry": "Cuba lagi",
  "updated": "Setakat"
},
    zh:{
  ...
  async function stockAging(root){
    ...
    const body=data.map((row,index)=>`<tr><td class="lineno">${index+1}</td><td class="l"><div class="cellsub"><b>${esc(row.productName)}</b><small>${esc(row.sku)} · ${esc(row.category)}</small></div></td><td class="l" data-business-text><b>${esc(row.warehouseCode)}</b><small>${esc(row.warehouseName)}</small></td><td class="l">${esc(date(row.lastInboundAt))}</td><td class="tnum">${row.ageDays==null?'—':`${integer(row.ageDays)} ${esc(c().days)}`}</td><td>${cap(bucketLabel(row.bucket),bucketTone(row.bucket))}</td><td class="tnum">${quantity(row.qty)}</td><td class="tnum"><b>${amount(row.inventoryValue)}</b></td></tr>`).join('');
