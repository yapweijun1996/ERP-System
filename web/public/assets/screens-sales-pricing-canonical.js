(function salesPricingCanonical(){
  ...
  function pack(packs){return i18nLegacy(packs);}
  ...
  const priceText={
    ...
    ms:{
  "title": "Senarai harga",
  "sub": "Harga standard dan pelanggan berkuat kuasa dengan lantai jualan terlindung.",
  ...
  "cancel": "Batal",
  "default": "Lalai",
  "empty": "Belum ada senarai harga kanonik.",
  "created": "Draf senarai harga dicipta",
  "activated": "Senarai harga diaktifkan"
},
  ...
  const discountText={
    ...
    ms:{
  "title": "Pengurusan diskaun",
  "sub": "Peraturan diskaun terkawal dengan tarikh kuat kuasa dan ambang kelulusan.",
  "new": "Peraturan baharu",
  "code": "Kod",
  "name": "Nama",
  "type": "Jenis",
  "standard": "Standard",
  ...
  "target": "Sasaran",
  "discount": "Diskaun",
  "approval": "Ambang kelulusan",
  "minOrder": "Pesanan minimum",
  "from": "Berkuat kuasa dari",
  "to": "Berkuat kuasa hingga",
  "status": "Status",
  "activate": "Aktifkan",
  "create": "Cipta draf",
  "cancel": "Batal",
  "empty": "Belum ada peraturan diskaun kanonik.",
  "created": "Draf peraturan diskaun dicipta",
  "activated": "Peraturan diskaun diaktifkan"
},
  ...
  SCREENS['price-lists']=async function(root){
    ...
    const columns=[
      ...
      {label:d.from,render:r=>`<span data-i18n-format="date" data-i18n-value="${esc(dateValue(r.effectiveFrom))}">${esc(formatDate(dateValue(r.effectiveFrom)))}</span>`},{label:d.lines,align:'r',render:r=>String(counts.get(Number(r.id))||0)},{label:d.status,render:r=>cap(r.status,tone(r.status))},
  ...
  SCREENS['discount-mgmt']=async function(root){
    ...
    const columns=[
      ...
      {label:d.from,render:r=>`<span data-i18n-format="date" data-i18n-value="${esc(dateValue(r.effectiveFrom))}">${esc(formatDate(dateValue(r.effectiveFrom)))}</span>`},{label:d.status,render:r=>cap(r.status,tone(r.status))},{label:'',align:'r',render:r=>r.status==='draft'?`<span class="rowact">${btn(d.activate,{icon:'check',cls:'primary',attrs:`data-activate-discount="${r.id}"`})}</span>`:''},
    ];
    function openCreate(){
  ...
  SCREENS['credit-control']=async function(root){
    const packs={
      ...
      ms:{
  "title": "Kawalan kredit",
  "sub": "Pendedahan invois belum dibayar dengan had dikuatkuasakan dan pegangan manual.",
  "customer": "Pelanggan",
  "limit": "Had kredit",
  "exposure": "Pendedahan",
  "available": "Tersedia",
  ...
  "status": "Status",
  "open": "Terbuka",
  "held": "Pegangan kredit",
  "new": "Profil kredit baharu",
  "currency": "Mata wang",
  "create": "Cipta",
  "cancel": "Batal",
  "hold": "Letak pegangan",
  "release": "Lepaskan",
  "reason": "Sebab pegangan",
  "empty": "Belum ada profil kredit kanonik.",
  "created": "Profil kredit dicipta",
  "heldMsg": "Pelanggan diletakkan dalam pegangan",
  "released": "Pegangan kredit dilepaskan"
},
