...
(function platformWorkspace(){
  ...
  function stepperMarkup(current){
    ...
    return `<ol class="platform-stepper" aria-label="Provisioning progress">${labels.map(function(label,index){ var number=index+1; var isComplete=number<current; var isCurrent=number===current; return `<li class="platform-step ${isComplete?'complete':''} ${isCurrent?'current':''}" aria-label="${esc(label)}" ${isCurrent?'aria-current="step"':''}><span>${isComplete?'✓':number}</span><b><span class="platform-step-label-full">${esc(label)}</span><span class="platform-step-label-short" aria-hidden="true">${esc(shortLabels[index])}</span></b></li>`; }).join('')}</ol>`;
  }
