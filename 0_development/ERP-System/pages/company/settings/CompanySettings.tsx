...
export const CompanySettings: React.FC = () => {
    ...
    const navItems = [
        { id: 'profile', label: t('settings.profile', 'Company Profile'), icon: Building, comp: CompanyProfileSettings },
        { id: 'fiscal', label: t('settings.fiscal', 'Fiscal & Tax'), icon: Receipt, comp: FiscalSettings },
        { id: 'numbering', label: t('settings.numbering', 'Document Numbering'), icon: FileText, comp: DocumentNumbering },
        { id: 'warehouse', label: t('settings.warehouse', 'Locations / Warehouses'), icon: MapPin, comp: () => <div className="p-8 text-center text-slate-500 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">Warehouse Configuration Module</div> },
        { id: 'approvals', label: t('settings.approvals', 'Approval Rules'), icon: ShieldCheck, comp: () => <div className="p-8 text-center text-slate-500 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">Approval Workflow Builder</div> },
        { id: 'payment', label: t('settings.payment', 'Payment Methods'), icon: CreditCard, comp: () => <div className="p-8 text-center text-slate-500 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl">Payment Gateway Setup</div> },
    ];

    const handleTabChange = (id: string) => {
        if (isDirty) {
            if (!window.confirm(t('common.unsaved_confirm', "You have unsaved changes. Are you sure you want to discard them?"))) {
                return;
            }
        ...
        <div className="flex flex-col md:flex-row gap-6 h-[calc(100vh-8rem)] pb-4 md:pb-6 pr-4 md:pr-6">
            ...
            <div className="flex-1 min-w-0 flex flex-col h-full overflow-hidden">
                <div className="mb-6 flex-shrink-0">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400">{t('settings.subtitle', 'Manage operational configurations for this entity.')}</p>
                </div>
