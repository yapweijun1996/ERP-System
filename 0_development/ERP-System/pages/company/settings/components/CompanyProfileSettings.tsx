...
export const CompanyProfileSettings: React.FC<{ onDirtyChange?: (isDirty: boolean) => void }> = ({ onDirtyChange }) => {
    ...
    const handleSave = () => {
        ...
        if (!formData.name) { addToast(t('common.validation_error', 'Validation Error'), t('common.error_name_required', 'Company Name is required.'), 'error'); return; }
        if (!formData.currency) { addToast(t('common.validation_error', 'Validation Error'), t('common.error_currency_required', 'Base Currency is required.'), 'error'); return; }
        ...
        setIsDirty(false);
        addToast(t('settings.profile_saved', 'Profile Saved'), t('settings.profile_saved_msg', 'Company settings updated successfully.'), 'success');
    };
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300 pb-24">
            ...
            <div className={`sticky top-0 z-30 transition-transform duration-300 ${isDirty ? 'translate-y-0' : '-translate-y-full'}`}>
                <div className="bg-amber-50 dark:bg-amber-900 border-b border-amber-200 dark:border-amber-800 px-6 py-3 flex items-center justify-between shadow-sm">
                    <div className="flex items-center text-amber-800 dark:text-amber-100 text-sm font-medium">
                        ...
                        {t('common.unsaved_changes', 'You have unsaved changes.')}
                    </div>
                    <div className="flex gap-2">
                        <button onClick={() => setIsDirty(false)} className="text-xs text-amber-700 dark:text-amber-300 hover:underline px-2">{t('common.discard', 'Discard')}</button>
                        <button onClick={handleSave} className="bg-amber-600 hover:bg-amber-700 text-white text-xs px-3 py-1.5 rounded font-medium transition-colors">{t('common.save_changes', 'Save Changes')}</button>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-5xl grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                        <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center">
                            <Building className="w-4 h-4 mr-2 text-blue-600" /> {t('settings.identity', 'Identity')}
                        </h3>
                        ...
                        <div className="mb-6 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg p-6 bg-slate-50 dark:bg-slate-950/30 hover:bg-slate-100 dark:hover:bg-slate-900 transition cursor-pointer group">
                            ...
                            <p className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('settings.upload_logo', 'Upload Logo')}</p>
                            <p className="text-[10px] text-slate-400 mt-1">PNG, JPG up to 2MB</p>
                        </div>

                        <div className="space-y-4">
                            <div className="space-y-1.5">
                                <label className={labelClasses}>{t('settings.company_name', 'Company Name')} <span className="text-red-500">*</span></label>
                            ...
                            <div className="space-y-1.5">
                                <label className={labelClasses + " flex justify-between"}>
                                    {t('settings.code_abbr', 'Code / Abbreviation')}
                                    <span className="text-[10px] normal-case font-normal text-slate-400">{t('settings.code_hint', 'For document prefixes')}</span>
                                </label>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                        <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center">
                            <Globe className="w-4 h-4 mr-2 text-blue-600" /> {t('settings.locale_regional', 'Locale & Regional')}
                        </h3>
                        <div className="space-y-4">
                            <div className="space-y-1.5">
                                <label className={labelClasses}>{t('settings.currency', 'Base Currency')} <span className="text-red-500">*</span></label>
                            ...
                            <div className="space-y-1.5">
                                <label className={labelClasses}>{t('settings.timezone', 'Timezone')}</label>
                            ...
                            <div className="space-y-1.5">
                                <label className={labelClasses}>{t('common.language', 'Language')}</label>
                ...
                <div className="xl:col-span-2 space-y-6">
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                        <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center">
                            <MapPin className="w-4 h-4 mr-2 text-blue-600" /> {t('settings.registered_address', 'Registered Address')}
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2 space-y-1.5">
                                <label className={labelClasses}>{t('settings.street_address', 'Street Address')}</label>
                    ...
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
                        <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-4 flex items-center">
                            <Phone className="w-4 h-4 mr-2 text-blue-600" /> {t('settings.contact_info', 'Contact Information')}
                        </h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1.5">
                                <label className={labelClasses}>{t('settings.general_email', 'General Email')}</label>
                            ...
                            <div className="space-y-1.5">
                                <label className={labelClasses}>{t('settings.phone_number', 'Phone Number')}</label>
                                <div className="relative">
                            ...
                            <div className="md:col-span-2 space-y-1.5">
                                <label className={labelClasses}>{t('settings.website', 'Website')}</label>
                                <div className="relative">
            ...
            <div className={`fixed bottom-0 right-0 left-0 md:left-64 p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center transition-transform duration-200 z-40 ${isDirty ? 'translate-y-0' : 'translate-y-full'}`}>
                <div className="text-sm text-slate-600 dark:text-slate-400 hidden sm:block">
                    ...
                    {t('common.updates_affect_docs', 'Updates will affect all generated documents immediately.')}
                </div>
                <div className="flex gap-4 ml-auto">
                    ...
                    >
                        <Save className="w-4 h-4 mr-2" /> {t('common.save_changes', 'Save Changes')}
                    </button>
                </div>
            </div>
        </div>
