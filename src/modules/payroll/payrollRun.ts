...
import {
  account, company, employee, glEntry, payrollRun, payrollRunLine,
} from '../../data/schema';
...
export async function createPayrollRunWithin(
  ...
  const lines = employees.map((row) => {
    const contributions = computeStatutoryContributions(country, row.baseSalary);
    return {
      employeeId: row.id,
      grossPay: row.baseSalary,
      ...contributions,
    };
  });
  ...
  await exec.insert(payrollRunLine).values(lines.map((line, index) => ({
    masterFn: scope.masterFn,
    companyFn: scope.companyFn,
    runId: run.id,
    lineNo: index + 1,
