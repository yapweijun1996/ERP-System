...
describe('sales return and credit note', () => {
  ...
  it('rejects a zero return quantity without creating a partial header', async () => {
    const db = await freshDb();
    const fx = await fixture(db);
    await expect(createSalesReturn(db, SCOPE, {
      docNo: 'RMA-ZERO',
