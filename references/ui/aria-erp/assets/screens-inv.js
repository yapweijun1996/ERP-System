...
SCREENS['stock-on-hand'] = function(root){
  ...
  function showDetail(sku){
    ...
    function tab(t){
      $$('#invTabs .tab').forEach(x=>x.classList.toggle('on',x.dataset.t===t));
      if(t==='overview'){
        body.innerHTML=`
          ...
          ${relatedDocs([{no:'SO-26-0418',label:'Sales order — Meridian',meta:'allocates 24 ea',status:'Pending Approval'},{no:'PO-26-0291',label:'Purchase order — inbound',meta:'+300 ea expected Jun 22',status:'Pending Approval'}])}`;
      } else if(t==='locations'){
