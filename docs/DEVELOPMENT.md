...
| `npm test` | **Vitest unit/integration suite** — domain transactions, API/auth, migrations, PGlite parity and conditional PostgreSQL security coverage. Current HEAD passes 172 files/697 tests with two intentional file/test skips. Most isolated tests use fresh PGlite state; the PostgreSQL suite requires its explicit URL/environment and otherwise records one conditional skip. |
...
access matrix pass as separate gates. Full local Vitest passes 172 files / 697 tests with
