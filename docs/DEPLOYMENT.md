...
Platform mobile header now compacts on scroll, preserves the tenant summary and Sign
out controls, and truncates long user names without horizontal overflow. Focused E2E,
lint, typechecks, Demo and production builds passed before release.
Evidence: [2026-09-24 production release](ai-native/evidence/PLATFORM-WORKSPACE-2026-09-24.md).
...
`34189671604` published the static Demo for the same HEAD. Its `release.json` reports the
same full revision, `fileCount: 133` and `dataMode: demo`. Later source
...
as HTTP 200. The latest manifest reports revision `ff6e0d9355c38ce06065267034c020ac55eea7e9`,
workflow `34189671604`, `fileCount: 133`, and `dataMode: demo`.
...
2. Create a fresh pre-deploy backup, apply migrations through 0103 with
...
All committed migrations through 0103 must be applied before a release; migration 0088
...
contracts; 0099–0103 provide the hidden Platform actor, governed tax, Receipt Pack
governance and bounded document-processing contracts.
