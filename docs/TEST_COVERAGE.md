...
Post-pull full regression — 2026-09-12: `npm test -- --reporter=dot` passed 211 test
files and 992 tests; four PostgreSQL-dependent files/tests skipped without
`POSTGRES_URL`. Exit 0 after 1957 seconds. This is local source evidence only; target
PostgreSQL, production, provider and physical-device evidence remain separate.
...
this does not prove SMTP or Platform Superadmin email recovery. Employee account
module/API regressions also cover malformed create/reset envelopes and malformed or
cryptographically tampered persisted temporary credentials returning bounded
unavailable responses without crypto detail.
...
35/48 DoD criteria** and **44/60 execution checkpoints**. TASK-227 verifies documentation, counts and links only;
...
TASK-234/S3 adds [Receipt Assistant loopback evidence](ai-native/evidence/TASK-234-2026-09-09.md#s3--implement-the-receipt-conversation-loop): six allowlisted server tool calls, cited Receipt facts, exact G06 preview/approval, persisted Pack and PDF artifact/source hash read-back, replay/no-duplicate behavior, denial/tenant-input rejection and cancel/no-write paths. The pilot inspection guard now also requires a non-empty one-to-one preview/detail identity set and rejects duplicate or extra successful detail identities before confirmation ([dated evidence](ai-native/evidence/TASK-234-2026-09-13-inspection-identity-guard.md)). S4 adds the contextual workspace and its five-locale/theme/desktop/mobile/focus matrix; S5 adds actual Demo/PGlite Pack/artifact assertions and local gates. No approved real-provider account or network model call is available, so that gate remains open.
...
Reviewed: **2026-09-13**, refreshed against the current GOAL/STATUS/PROGRESS snapshots
and dated task follow-up evidence. This remains an evidence map, not a replacement
for the individual dated task records.
...
or passing tests. The latest full Vitest run passes 211 files / 992 tests with 4
PostgreSQL-dependent files/tests skipped. Source folder links provide the current implementation and
...
| [documents](../src/modules/documents/) | 5 | Route rendering only; focused coverage protects the shared scanner/OCR/Vision URL and redirect guards, bounded response-body parsing, failed-response body cleanup, provider visual-fingerprint validation/persistence, the actual HTTP Vision 401/500 driver-to-worker failure boundary, bounded worker error persistence, the tenant-scoped audited dead-letter retry action and the secret-free document-processing readiness projection/API. Readiness derives Company and Master scope from the authenticated session, requires paired `lastCheckedAt`/`lastSuccessAt` health evidence, revalidates required persisted credential envelopes and rejects malformed stored values at readiness, policy selection and worker extraction, covers cross-Master and client-query tamper isolation, and invalidates stale health after connector pause or missing health-check evidence, while external scanning/OCR/storage/provider and delivered alert/recovery evidence remains TASK-205. |
...
| PWA update lifecycle | v279 silent activation/reload flow is covered by `npm run audit:pwa-update` | Physical devices, multiple tabs, unsaved drafts, in-flight requests and interrupted upgrades remain unverified |
...
| Worker telemetry | Focused telemetry tests pass 9/9; ready/in-flight predicates cover active leases, document processing claim states, enabled calendar connections and reminder due time; calendar leave/appointment aggregates use a tenant-scoped derived enabled-connection `LEFT JOIN`; emission is single-flight/non-blocking, snapshots record `queryDurationMs`, optional transaction-local query timeout is bounded, `check:worker-telemetry` validates captured aggregate NDJSON with an explicit threshold, and an isolated PostgreSQL 16 probe confirms `statement_timeout` cancellation/reset | TASK-201; join optimization, timeout configuration, duration, checker and local cancellation/query-plan observations are source/local evidence while representative whole-table query budget/plan evidence, operational sink/alerts and production SLO/DR/load evidence remain |
| Release identity and read-only release evidence | `scripts/write-release-manifest.test.ts` passes 4/4 for replacement, `0600` permissions, cleanup, failure preservation and symlink/non-file rejection; `scripts/verify-release.test.ts` passes 6/6 against a local HTTP fixture for root/health/setup/manifest, asset byte/hash matching, revision mismatch, same-path redirect acceptance, changed-path rejection and CLI exit/status parity; `scripts/rollback-release.test.ts` passes 5/5 for immutable plan, unsafe-reference and confirmation guards plus fake-Docker execute scoping/cleanup; `scripts/check-availability.test.ts` passes 15/15 for healthy/degraded classification, fixed-message redaction, deterministic recovered/degraded retry dedupe, idempotency-key forwarding, HTTPS host allowlisting, unsafe endpoint rejection, response cleanup, timeout and CLI argument redaction; `scripts/check-availability-alert.test.ts` passes 7/7 for dry-run, allowlisted delivery, missing configuration, sink rejection, timeout and CLI redaction | Run `npm run verify:release -- <origin> --expected-revision <commit>` or the bounded `npm run check:availability -- <origin> --expected-revision <commit>` against the selected release origin; `npm run check:availability-alert -- <origin> --expected-revision <commit>` is the scheduler handoff and remains dry-run without explicit endpoint/allowlist; live deployed `/health` + `/release.json` and every listed asset hash/byte count must match, while remote alerting, incident ownership and exercised rollback evidence remain TASK-199 |
| Full unit/integration suite | Post-pull workspace passes 211 files / 992 tests with 4 PostgreSQL-dependent files/tests skipped; TASK-205 readiness, TASK-234 assistant runtime/loop and TASK-235 semantic/knowledge regressions are included | PostgreSQL target and production evidence remain separate |
...
| TASK-229/S5 browser regressions and local gates | Authenticated API/PGlite E2E, temporary 375px screenshot inspection, contrast (2 themes × 2 viewports), five-locale live i18n, mobile usability, latest full Vitest (211/992 with 4 PostgreSQL-dependent files/tests skipped), Demo/build, schema/pack/i18n/drift/RLS/permissions, root/Web typecheck, lint/docs/diff all pass; `npm run test:e2e:webmcp-native` passes Chrome 152 native registration/invocation, cancellation/confirmation, Pack/PDF persistence and mobile bounds | Native local/repository scope is accepted; production browser flag/release, PostgreSQL target, provider and physical-device evidence remain open |
...
| TASK-234/S2 secret-safe provider configuration | Company-scoped `agent_provider_config` stores bounded provider/model/data-policy/runtime metadata and AES-GCM credentials; explicit rotation/provider-change decisions, allowlisted models, exact HTTPS egress, tenant-derived API scope, permission/idempotency boundaries and secret-free views/audits pass 2 files / 7 focused tests. Persisted credential envelopes are revalidated at public view/update boundaries; a malformed value is unconfigured/disabled and cannot be retained without repair. The focused provider/API/assistant regression passes 3 files / 30 tests; migration 0110, generated schema, drift, permissions and production-RLS checks pass | Real provider account/network evidence, receipt conversation, contextual UI, locale/theme/mobile accessibility and production readiness remain S3-S5 |
...
TASK-236 is Done for repository scope; TASK-231, TASK-234 and TASK-237–239 remain AI Native delivery work. Current registry:
