...
Status: the initial API, data and human-triage slice was deployed on 2026-09-24 at revision `b8c0208aae430d26d672ce10f2a62ad88f068f71`. A 2026-09-25 source candidate adds bounded Agent evidence, tracked task linkage, release proof, independent verification, guarded closure and reporter readback; it is not deployed or accepted in production. The production migration, forced RLS, immutable-event trigger and public release verifier passed for the initial slice only. No Agent principal or product-case grant existed in production at that time, so no internal Agent was activated and no live submission was made. [EPIC-069](EPICS.md) and TASK-244–249 in [`tasks/tasks.jsonl`](../tasks/tasks.jsonl) own the remaining acceptance and rollout. Named assignees and service targets remain to be set by the product owner.

## Source candidate (2026-09-25; not yet deployed)

- `POST /api/agent/cases/{id}/evidence` accepts a bounded `observation` or `reproduction` note from the submitting Agent with the separate `product_case.append_evidence` grant and a scoped `Idempotency-Key`. Exact retries return the original evidence; changed payloads conflict. `GET /api/agent/cases/{id}` can return reporter-visible evidence, status, release revision, verification time and final reason subject to the Agent's field allowlist. Existing grants do not expand automatically.
- Human `POST /api/product-cases/{id}/triage` classifies a case and optionally assigns an active Company user. `/{id}/tasks` links a stable engineering task reference. `/{id}/evidence` records a bounded human reproduction note. `/{id}/transitions` now enforces `needs_info`, `accepted`, task-before-resolution, reason-coded non-fix closure, same-Company duplicate links, version checks and reopen.
- `POST /api/product-cases/{id}/releases` requires `product.cases.release`, a resolved case, and a linked task. The server, not the browser, verifies its configured public origin against the running revision and asset manifest before storing the revision and a proof digest. `POST /api/product-cases/{id}/verifications` requires `product.cases.verify`, a released case and a different human from both the task linker and releaser. A failed verification returns the case to work; a passed verification permits fixed closure. Case projection, append-only evidence/event and central audit commit together.
- `product.cases.evidence_append`, `product.cases.release` and `product.cases.verify` are explicitly assigned permissions, excluded from the Company Owner default bundle. A named Agent owner needs the first permission before an evidence grant can be issued; release and verification must be assigned to accountable human roles separately. Upgrading does not silently expand existing owner authority.
- The API-mode Admin > Product Cases screen exposes classification, task/evidence entry, release, verification and guarded closure. Demo mode remains read-only/API-only for this governed workflow. `product_case_evidence` has Company RLS and a production UPDATE/DELETE rejection trigger. The migration and generated Demo schema must be applied before this candidate can run in production.
- Deliberate limits: no arbitrary attachment upload, no automatic external task-registry validation, no push notification, no distributed rate limiter and no activated Agent pilot. Outcome readback is authenticated pull, not a delivery guarantee. Product/security owners must still approve budgets, retention, permissions, on-call ownership and the internal canary before claiming full-loop acceptance.
...
## Candidate and remaining target interfaces
...
| `GET /api/agent/cases/{id}` | Submitting Agent with `product_case.read_own` grant | Source candidate: current state, safe status history, reporter evidence and released/verified/final outcome fields when explicitly allowed by the grant. |
| `POST /api/agent/cases/{id}/evidence` | Submitting Agent with `product_case.append_evidence` grant | Source candidate: append one bounded text observation or reproduction note and return its ID; retries are idempotent. File uploads remain unsupported. |
| `/api/product-cases` reads and human command routes | Human session with dedicated product-case permissions | Source candidate: Company-scoped queue, detail/history, triage, task/evidence linkage, release proof, independent verification and guarded status transitions. |
...
Candidate states: `submitted -> triaged -> accepted -> in_progress -> resolved -> released -> verified -> closed`. `needs_info` pauses triage. A triaged case may close with a reason such as `duplicate`, `not_reproducible`, `declined` or `answered`. A fixed case needs a linked task and server-verified deployed revision before `released`, independent post-release evidence before `verified`, and an outcome available to the reporter before `closed`. Reopening appends an event and returns the case to triage without erasing earlier release or verification facts; the current projection is cleared for a new cycle. `duplicate_of_id` may point only to an earlier, non-duplicate case in the same Company.
