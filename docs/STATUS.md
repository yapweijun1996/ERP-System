...
The full Vitest regression is also green: 154 files, 623 tests passed, one intentional
skip, zero failures (624 test slots). Assignment/RBAC/admin/manager and approval-focused
tests pass, including expiry, revoke, multi-target, explicit deny precedence and safe
...
| Canonical UI i18n | ✅ Five-language release proven | 1,297-key canonical resources plus 73 registered module packs provide en/ms/zh/ja/vi across all 125 Canonical routes. `setLang()` is atomic and state-preserving; `npm run audit:i18n` proves 125 routes × 5 languages × desktop/375px with no blocking findings. TASK-137–139 and TASK-156. |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 125 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **125 Canonical / 0 Preview**. Preview pages, if reintroduced by a future task, distinguish Sample Data from Canonical Data and lock write-like actions. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Green | 2026-08-10: the full Vitest run covers 154 files; 623 passed, one intentional skip, zero failed (624 test slots). The permission registry, resource/action contracts, Demo v15 pack, permission/module tests and authenticated integration coverage agree with the current application catalog. |
...
125 routes are registered in the live `SCREENS` registry. `SCREEN_META` is the source
of truth for production maturity at route level: **125 routes are Canonical and 0 are
Preview**. The independent desktop/375px screen audit and the five-language matrix both
reconfirm this boundary.
...
connector surface; the current route boundary is 125 Canonical / 0 Preview.
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 125 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; recipient-addressed notifications joined in EPIC-048. |
| Every Canonical route has five-language coverage | **Implemented and globally proven.** `npm run audit:i18n` validates resources, placeholders and unsafe markup, then renders 125 routes × en/ms/zh/ja/vi × desktop/375px while checking raw keys, hardcoded system copy, runtime errors and horizontal overflow. |
...
generated/exported/statutory documents remain outside UI i18n. The current automated
proof covers 125 routes × five languages × desktop/375px; current PWA cache version is
v259.
