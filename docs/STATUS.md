...
| Canonical schema (119 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 41 ordered migrations through `drizzle/0040_modern_jimmy_woo.sql`, `src/data/schema/` |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **107 Canonical / 7 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 348 passing tests plus 1 expected skip and one gated PostgreSQL 16 integration proof (also run in CI). Coverage includes Session/CSRF/RBAC, idempotency/audit, inventory/warehouse/manufacturing/quality invariants, sales/purchasing/CRM lifecycles, finance/assets/project/service/HR/payroll postings, rebuildable BI and migration compatibility. Timesheet additionally proves actor isolation, Decimal/date/state validation, Viewer denial, audit and idempotent append-preserving void. |
...
| `VITE_DATA_MODE=api` renders every current Canonical route | ✅ Working | `erp-system-api-adapter.js` calls the authenticated API with no sample fallback. Dashboard, Inventory, Warehouse, Manufacturing, Quality, Purchasing, CRM, Sales, Finance, Project, Reporting/BI and Settings all consume canonical resources/session data. **107 of 107 current Canonical routes support both Demo and API mode**; Timesheet joined in EPIC-044/TASK-080. Company switching re-fetches the authenticated tenant scope. |
...
| Service worker never caches `/api/*` or `/health` | ✅ Working | `web/public/sw.js` (`CACHE_VERSION` v68) — the Cache API keys purely on URL and ignores cookies, so caching session-scoped responses could serve a stale signed-in state after logout. |

## Canonical and Preview route boundary

114 routes are registered in the live `SCREENS` registry. `SCREEN_META` is now the source
of truth for production maturity at route level: **107 routes are Canonical and 7 are
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 107 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; Timesheet joined in EPIC-044. |
