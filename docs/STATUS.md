...
| Canonical schema (54 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | Ordered migrations through `drizzle/0009_wet_nekra.sql`, `src/data/schema/` |
...
| Manufacturing work-order foundation and execution | ✅ Canonical Demo/API data and writes | Migration 0009 adds tenant-scoped work centres, versioned BOM/components, routings/operations and work-order material/operation snapshots. Shared Decimal-based commands create/release, issue all material through the inventory ledger, report operations in sequence and atomically receive finished goods. Material issue posts Dr WIP/Cr Inventory; completion posts Dr Inventory/Cr WIP. Domain and authenticated HTTP tests cover shortage rollback, duplicate/replayed actions, operation gates, stock conservation and GL balance. `work-orders`, `work-order` and `new-work-order` use only bounded formal resources in Demo/API. Returns, partial completions, labour/overhead and MRP remain the next manufacturing slices. |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **26 Canonical / 88 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 113 passing tests plus one gated PostgreSQL 16 integration proof. Includes persistent Session restart, CSRF, RBAC, encrypted account lifecycle, setup, atomic action idempotency/replay/expiry, inventory adjustment snapshot conflicts, transfer conservation, bin/lot/serial invariants, manufacturing create/release/issue/report/complete rules, complete inventory, purchasing and CRM resource/transaction proofs, audit correlation and migration compatibility. |
...
| `VITE_DATA_MODE=api` renders every current Canonical route | ✅ Working | `erp-system-api-adapter.js` calls the authenticated API with no sample fallback. Dashboard, Inventory, Warehouse, Manufacturing, Purchasing, CRM, Sales, Finance and Settings all consume canonical resources/session data. **26 of 26 current Canonical routes support both Demo and API mode.** Company switching re-fetches the authenticated tenant scope. |
...
of truth for production maturity at route level: **26 routes are Canonical and 88 are
...
**Manufacturing is partially converted (2026-07-19).** Work-order list, detail and
creation/release and execution use the canonical versioned BOM/routing snapshot model in
Demo and API modes. Material issue, sequential operation reporting, finished-goods receipt
and material WIP/Inventory GL are live. BOM authoring, returns, partial completion,
labour/overhead costing and MRP run commands are not complete, so `bom` and `mrp` remain Preview.

## Documented but NOT implemented (do not assume these exist)

| Claim in docs | Reality |
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 26 current Canonical routes use `ErpSystemData` in API mode with no sample fallback. Settings reads the authenticated session and labels browser-local preferences honestly. Item master, BOM/MRP, quality and CRM detail/Customer 360 remain Preview until their schema, adapters, permissions, tests and five-language coverage meet the Canonical gate. |
| API server has all business **write** endpoints | Not yet. Production setup, auth lifecycle, CRM opportunity conversion, Sales Draft confirmation, inventory adjustment post, stock-transfer completion, work-order create/release, PO creation/receipt and supplier-invoice posting are live; advanced warehouse, manufacturing execution/completion, quality and remaining finance/commercial actions still need registration on the unified dispatcher. |
