...
| PWA (manifest, SW, update prompt, safe areas) | ✅ Working | `web/public/manifest.webmanifest`, `sw.js`, `pwa.js`; v138 precaches the English i18n bootstrap and business boundary, then runtime-caches successfully loaded non-English packs. |
| Canonical UI i18n | ✅ Five-language release proven | 1,141-key canonical resources plus 73 registered module packs provide en/ms/zh/ja/vi across all 122 Canonical routes. `setLang()` is atomic and state-preserving; `npm run audit:i18n` proves 122 routes × 5 languages × desktop/375px with no blocking findings. TASK-137–139. |
...
| Full screen audit — every route in `SCREENS` (122), desktop + 375px | ✅ Demo / ⚠️ API expected-conflict noise | Demo screen and five-language audits pass at 122 Canonical / 0 Preview. The 1,220-case API matrix has zero blank/overflow/session-loss states; five unlinked-employee My Work routes repeat expected 409s and console noise (TASK-148). |
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ⚠️ Known baseline failures | 2026-07-27 `npm test`: 505 pass, 7 fail, 1 skip. Six document-processing tests are wall-clock-dependent (TASK-144); one session restart hook timeout passes focused and groups with parallel setup contention (TASK-146). Business-focused reruns pass. |
...
| PostgreSQL concurrency/parity proof | ✅ Working on dedicated proof DB | PGlite/PostgreSQL business results match and one of two stock races wins; RLS proof passes. A seeded-database safety guard remains TASK-147. |
| `VITE_DATA_MODE=api` renders every current Canonical route | ✅ Rendered / ⚠️ permission-shell UX | All 122 routes render in API mode with no sample fallback. Pure Employee shell is restricted; Manager/custom Finance navigation is broader than effective permissions even though direct APIs fail 403 (TASK-145). Unlinked My Work expected 409s remain TASK-148. |
...
| Service worker never caches `/api/*` or `/health` | ✅ Working | `web/public/sw.js` (`CACHE_VERSION` v138) keeps session-scoped API/health responses out of Cache API while caching static English i18n and successfully fetched non-English resource packs. |

## Canonical and Preview route boundary

122 routes are registered in the live `SCREENS` registry. `SCREEN_META` is the source
of truth for production maturity at route level: **122 routes are Canonical and 0 are
...
| Every Canonical route has five-language coverage | **Implemented and globally proven.** `npm run audit:i18n` validates resources, placeholders and unsafe markup, then renders 122 routes × en/ms/zh/ja/vi × desktop/375px while checking raw keys, hardcoded system copy, runtime errors and horizontal overflow. |
...
generated/exported/statutory documents remain outside UI i18n. Final automated proof
covers 122 routes × five languages × desktop/375px; PWA cache version is v138.
