...
| Canonical schema (110 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 37 ordered migrations through `drizzle/0036_lively_captain_cross.sql`, `src/data/schema/` |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **98 Canonical / 16 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 306 passing tests plus 1 expected skip and one gated PostgreSQL 16 integration proof (also run in CI). Coverage includes Session/CSRF/RBAC, idempotency/audit, inventory/warehouse/manufacturing/quality invariants, sales/purchasing/CRM lifecycles, finance/assets/project/service/HR/payroll postings and migration compatibility. Sales-order approval adds Decimal tax, tenant/actor guards, replay, Viewer denial, audit and no-stock/no-GL proofs. |
...
offer no fake export or queued-report action. Commission and the shared prototype
`txn-view` remain Preview.

## Documented but NOT implemented (do not assume these exist)

| Claim in docs | Reality |
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 98 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; Sales analytics joined in EPIC-037. |
