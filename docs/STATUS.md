...
The current `npm run audit:i18n` run covers the complete 128-route × 5-language ×
2-viewport browser matrix and all routes render, but the static audit exits non-zero
with 263 blocking findings. The findings are missing local-pack keys plus
hardcoded/dynamic user-facing strings; the localization release gate is therefore not
green even though the screen route audit is green.
...
| Canonical UI i18n | ⚠️ Browser matrix renders; static audit blocked | 1,297-key canonical resources plus 73 registered module packs provide en/ms/zh/ja/vi. On 2026-08-10, `npm run audit:i18n` exercised all 128 routes × 5 languages × 2 viewports and rendered the matrix, but exited with 263 blocking findings from missing local-pack keys and hardcoded/dynamic user-facing strings. `setLang()` remains atomic and state-preserving; localization remediation is an open release-quality slice. |
...
desktop/mobile layout/behavior contracts. The same date's full i18n browser matrix
renders all routes/languages/viewports, but the static audit remains blocked by 263
findings; the last fully green five-language proof still covered 125 routes.
...
| Every Canonical route has five-language coverage | **Not currently proven.** The 2026-08-10 matrix covers 128 routes × 5 languages × 2 viewports and renders, but `npm run audit:i18n` exits with 263 blocking static findings from missing local-pack keys and hardcoded/dynamic text. |
