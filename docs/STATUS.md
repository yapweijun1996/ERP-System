...
| Full screen audit — every route in `SCREENS` (122), desktop + 375px | ✅ Working | `scripts/audit-screens.mjs`, `npm run audit:screens`; all routes render without console/page errors, route maturity is 122 Canonical / 0 Preview, and layout, stateful detail, shared-shell and action-bar contracts pass. |
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`: 512 passing tests plus 1 expected skip across 132 files. Added focused i18n atomic-load/fallback/safety/formatting tests and structured API-error compatibility tests. |
...
| `VITE_DATA_MODE=api` renders every current Canonical route | ✅ Working | `erp-system-api-adapter.js` calls the authenticated API with no sample fallback. All **122 current Canonical routes** support Demo/API and the route audit reports 0 Preview. An Employee-only session can boot its restricted shell without `dashboard.read`; company switching re-fetches the authenticated tenant scope and capability context. |
