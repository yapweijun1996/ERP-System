# Project Status — reviewed 2026-07-22
...
| Canonical schema (107 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 35 ordered migrations through `drizzle/0034_dapper_karnak.sql`, `src/data/schema/` |
...
| Purchasing chain: requisition/RFQ/quote → PO approval → receipt/invoice → return/credit/debit/landed cost, end-to-end incl. screens | ✅ Canonical Demo/API data and writes | Requisitions, RFQs, invited supplier quotations and the PO/receipt/invoice chain use bounded formal resources in both modes. RFQ award creates one linked pending-approval PO without stock/GL effects; an authorised decision must open it before receipt. Receipt uses `receiveStockWithin` and updates moving-average cost; supplier-invoice posting creates balanced GL. A receipt/invoice-linked purchase return atomically issues stock and creates an immutable supplier credit. Commercial supplier debit notes post Dr AP / Cr Purchase Variance / Cr Input Tax without stock movement. Landed cost allocates tax-exclusive freight/duty/handling by receipt value or quantity, revalues current on-hand and posts Dr Inventory / Cr Landed Cost Accrual without changing quantity. Supplier price lists and vendor performance remain Preview. |
...
| Browser smoke test (desktop + mobile, zero console/page errors, dashboard content verified) | ✅ Working | `scripts/smoke.mjs`, `npm run smoke`, Playwright, wired into CI with browser caching, TASK-015. The browser gate awaits the adapter's bounded readiness promise before starting its UI timeout, so a slow GitHub runner cannot race PGlite's 20-second cold-start watchdog. |
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **80 Canonical / 34 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Purchase Requisition: register, approval & real PO linkage | ✅ Canonical Demo/API data and writes | `purchase_requisition`/`purchase_requisition_line` and the nullable `purchase_order.requisition_id` provide real submitted→approved/rejected decisions and traceable conversion. The new sourcing chain can alternatively carry an approved requisition into an RFQ; direct PO conversion is then blocked so supplier comparison cannot be bypassed. RFQs, supplier quotations, returns, supplier notes and landed cost are Canonical; supplier price lists and vendor performance remain Preview. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 289 passing tests (1 skipped) plus one gated PostgreSQL 16 integration proof (also run in CI). Coverage includes Session/CSRF/RBAC, idempotency/audit, inventory/warehouse/manufacturing/quality invariants, sales/purchasing/CRM lifecycles, finance/assets/project/service/HR/payroll postings and migration compatibility. PO approval coverage adds receipt gating, actor/company validation, Viewer denial, replay/audit, terminal-state rollback and no-stock/no-GL proofs. |
...
| Service worker never caches `/api/*` or `/health` | ✅ Working | `web/public/sw.js` (`CACHE_VERSION` v52) — the Cache API keys purely on URL and ignores cookies, so caching session-scoped responses could serve a stale signed-in state after logout. |

## Canonical and Preview route boundary

114 routes are registered in the live `SCREENS` registry. `SCREEN_META` is now the source
of truth for production maturity at route level: **80 routes are Canonical and 34 are
...
before receipt, without moving stock or writing GL. Supplier price lists, vendor
performance, purchasing reports and the shared `pur-txn-view` remain Preview.
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 80 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; purchasing transaction details joined in EPIC-033. |
...
- Done: TASK-001…016, TASK-018…069 (68)
- Blocked: TASK-017 (1)
- Todo in the historical task registry: none. The remaining product work is the 34
