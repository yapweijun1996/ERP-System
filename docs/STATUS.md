...
| Canonical schema (122 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 43 ordered migrations through `drizzle/0042_lovely_ma_gnuci.sql`, `src/data/schema/` |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **110 Canonical / 4 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 360 passing tests plus 1 expected skip and one gated PostgreSQL 16 integration proof (also run in CI). Coverage includes Session/CSRF/RBAC, idempotency/audit, inventory/warehouse/manufacturing/quality invariants, sales/purchasing/CRM lifecycles, finance/assets/project/service/HR/payroll postings, rebuildable BI and migration compatibility. Timesheet proves actor isolation; integration tests prove sanitized event visibility plus bounded customer-import validation, rollback, tenant/RBAC/audit/idempotency and newest-first pagination. |
...
| Service worker never caches `/api/*` or `/health` | ✅ Working | `web/public/sw.js` (`CACHE_VERSION` v71) — the Cache API keys purely on URL and ignores cookies, so caching session-scoped responses could serve a stale signed-in state after logout. |

## Canonical and Preview route boundary

114 routes are registered in the live `SCREENS` registry. `SCREEN_META` is now the source
of truth for production maturity at route level: **110 routes are Canonical and 4 are
...
`master-control` and `sys-settings` remain Preview; `notifications` and the connector
hub are the other two remaining Preview routes.
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 110 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; actor-owned personal activity joined in EPIC-047. |
...
- Done: 82 tasks, including TASK-083 (all registered tasks except TASK-017)
- Blocked: TASK-017 (1)
- Todo in the historical task registry: none. The remaining product work is the 4
