...
Current verification on 2026-08-10: root/Web typecheck, ESLint, Demo build, generated
Demo schema, 244-table drift, Demo-pack and permission-registry checks pass. Focused
...
Canonical/0 Preview with no console/page errors, but its release layout/behavior gate
currently fails 17 assertions: Goods Receipt, Supplier Price Lists, Supplier Quotations,
Bank Reconciliation, Employee, Receipt & Tax Evidence, Sales Reports, Service Contract,
Stock Aging and Stock-on-Hand active-subnav visibility; HR Calendar detail-region order;
Leave Approval's pending Approve action; and My Work's employee tab count. These are
current working-tree regressions, not TASK-017's physical-device blocker and not an
authorization-matrix failure; they must be assigned and fixed before a release is called
fully green.
...
| Sales order authoring and approval | ✅ Canonical Demo/API data and writes | Migration 0036 adds one versioned approval per order; migration 0078 adds canonical `stock`/`non_stock` line identity, description and UoM snapshots. Direct and quotation-converted orders start `pending_approval`; create snapshots effective tax and approve/reject requires an active company actor plus auditable note. Approval releases only a `draft` and produces no stock, delivery, invoice or GL fact. `new-sales-order` supports inline stock-item creation and free-text service rows; confirmation creates delivery/stock effects only for stock lines while service lines still invoice and post revenue. `new-sales-order` and `so-approvals` are bounded five-language Canonical routes. |
...
| Canonical UI i18n | ⚠️ Historical proof needs rerun after the current route expansion | 1,297-key canonical resources plus 73 registered module packs provide en/ms/zh/ja/vi. The last green proof covered 125 routes; the current screen registry is 128 routes and the full 128-route i18n proof must be rerun after the current screen regressions are resolved. `setLang()` remains atomic and state-preserving. |
...
| Full screen audit — every route in `SCREENS` (128), desktop + 375px | ⚠️ Current working-tree regression | All 128 routes render and the maturity contract passes at 128 Canonical / 0 Preview, but 17 layout/behavior assertions fail; see the 2026-08-10 source-of-truth synchronization above. Permission-aware navigation and the separate access-matrix API/browser checks remain green. |
...
Preview**. The screen audit reconfirms route rendering and maturity; its release
layout/behavior gate currently has 17 failures. The last green five-language proof
covered 125 routes and must be rerun for the current 128-route registry.
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Contract implemented for the present Canonical boundary; full current API browser proof pending.** The current 128-route screen audit is the Demo/PGlite run and has 17 release layout/behavior failures; the access matrix covers API/browser permission boundaries. Run a dedicated API-mode layout audit before release. |
| Every Canonical route has five-language coverage | **Last green proof covered 125 routes.** The current registry has 128 routes; rerun `npm run audit:i18n` after the current screen regressions are fixed to re-prove resources, placeholders, unsafe markup, runtime errors and overflow across en/ms/zh/ja/vi. |
...
generated/exported/statutory documents remain outside UI i18n. The last green automated
proof covered 125 routes; rerun the full 128-route matrix after the current screen
regressions are fixed. Current PWA cache version is v259.
...
The dependency order is TASK-173 approval-authority unification and remaining decision
