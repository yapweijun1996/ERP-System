...
| Purchasing chain: PO → goods receipt (stock IN) → supplier invoice (balanced GL), end-to-end incl. screens | ✅ Working | `src/data/schema/purchasing.ts`, `src/modules/purchasing/` (`createPurchaseOrderWithin`/`receiveGoodsWithin`/`postSupplierInvoiceWithin`) now run unchanged through the bundled Demo ESM runtime and server adapters. The browser proof covers stock +2, balanced AP GL and duplicate-invoice rejection; a PO row lock plus an in-transaction existing-invoice check serializes competing requests without imposing a destructive migration on existing tenants. RFQs/quotations/requisitions/returns/credit-debit-notes/price-lists/landed-cost/vendor-performance have no schema and stay mock. |
...
| Bundled Demo ESM runtime | ✅ CRM + Purchasing migrated | `web/src/erp-demo-runtime*.ts` bundles PGlite, Drizzle, canonical schema and shared domain commands locally. CRM create/convert and Purchasing create/receive/post use the shared TypeScript commands; Sales/Setup compatibility SQL still awaits migration. API builds remove this entry before bundling, so production web artifacts contain no PGlite WASM/data payload. The service worker discovers and precaches the Demo build's content-hashed runtime/WASM/data graph for offline reuse. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 76 PGlite tests plus one gated PostgreSQL 16 integration proof. Includes persistent Session restart, CSRF, RBAC, encrypted account lifecycle, setup, atomic action idempotency/replay/expiry, audit correlation and migration compatibility. |
...
| Production canonical resource API | ✅ Read platform + first action | `src/api/resources.ts` declares table/id/scope/permissions/filter/sort/action/version/idempotency/audit metadata. Lists use opaque keyset cursors and `limit≤100`; versioned details return quoted ETags. The unified transactional action dispatcher now powers idempotent/audited CRM opportunity conversion; remaining business actions are still pending. |
...
| API server has all business **write** endpoints | Not yet. Production setup, auth lifecycle and CRM opportunity conversion are live; sales, purchasing, inventory and finance actions still need registration on the unified dispatcher. |
...
1. **Seed/domain SQL duplication remains, but the migration has started.** Browser PGlite schema and compatibility
   ...
   domain commands vs the demo adapter's Sales/Setup raw business SQL mirrors are still
   manually duplicated. CRM and Purchasing now use the shared commands through
   `web/src/erp-demo-runtime-impl.ts`.
