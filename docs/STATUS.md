...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **78 Canonical / 36 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Service worker never caches `/api/*` or `/health` | ✅ Working | `web/public/sw.js` (`CACHE_VERSION` v50) — the Cache API keys purely on URL and ignores cookies, so caching session-scoped responses could serve a stale signed-in state after logout. |

## Canonical and Preview route boundary

114 routes are registered in the live `SCREENS` registry. `SCREEN_META` is now the source
of truth for production maturity at route level: **78 routes are Canonical and 36 are
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 78 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; PO approvals joined in EPIC-032. |
...
- Done: TASK-001…016, TASK-018…068 (67)
- Blocked: TASK-017 (1)
- Todo in the historical task registry: none. The remaining product work is the 36
