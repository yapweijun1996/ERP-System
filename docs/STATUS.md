...
696 tests with two intentional file/test skips. TASK-204 source-level tax interval,
...
local full Vitest result is 172 files / 696 tests with two intentional file/test skips.
TASK-017 remains
...
2026-09-07 current run passes 172 files / 696 tests with two skips. The authenticated `account/*`
...
| Canonical schema (255 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 103 ordered migrations through schema version 102; `drizzle/`, `src/data/schema/`; migration 0099 adds the Platform tenant actor/elevated-session foundation, migrations 0100/0101 add governed tax facts and Expense tax classification, and migration 0102 adds Receipt Pack governance and Company timezone facts |
