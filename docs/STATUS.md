...
| Canonical schema (114 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 38 ordered migrations through `drizzle/0037_acoustic_jigsaw.sql`, `src/data/schema/` |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **99 Canonical / 15 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 317 passing tests plus 1 expected skip and one gated PostgreSQL 16 integration proof (also run in CI). Coverage includes Session/CSRF/RBAC, idempotency/audit, inventory/warehouse/manufacturing/quality invariants, sales/purchasing/CRM lifecycles, finance/assets/project/service/HR/payroll postings and migration compatibility. Sales commission adds Decimal source reconciliation, immutable ownership/rate snapshots, overlap/tenant guards, Viewer denial, replay, audit and no-payroll/no-GL proofs. |
...
| Service worker never caches `/api/*` or `/health` | ✅ Working | `web/public/sw.js` (`CACHE_VERSION` v59) — the Cache API keys purely on URL and ignores cookies, so caching session-scoped responses could serve a stale signed-in state after logout. |

## Canonical and Preview route boundary

114 routes are registered in the live `SCREENS` registry. `SCREEN_META` is now the source
of truth for production maturity at route level: **98 routes are Canonical and 16 are
...
offer no fake export or queued-report action. The shared prototype `txn-view` remains Preview.
...
does not post payroll, payment or GL. Only the shared prototype `txn-view` remains
Preview in Sales.
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 99 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; Sales commission joined in EPIC-038. |
...
- Done: TASK-001…016, TASK-018…073 (72)
- Blocked: TASK-017 (1)
- Todo in the historical task registry: none. The remaining product work is the 16
