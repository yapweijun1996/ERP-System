...
through an audited idempotent action. Purchasing's RFQs/quotations remain mock. What's
...
of truth for production maturity at route level: **70 routes are Canonical and 44 are
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 70 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`. Settings reads the authenticated session and labels browser-local preferences honestly; CRM opportunity detail joined the boundary in EPIC-027. |
