...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 69 current Canonical routes use `ErpSystemData` in API mode with no sample fallback (this count was already stale — 58 — before EPIC-026 touched it; corrected to the real current total, confirmed via `npm run audit:screens`). Settings reads the authenticated session and labels browser-local preferences honestly. CRM's opportunity-detail sub-screen remains Preview (no schema). |
