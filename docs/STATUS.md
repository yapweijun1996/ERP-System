...
| Manufacturing work-order foundation | ✅ Canonical Demo/API data and writes | Migration 0009 adds tenant-scoped work centres, versioned BOM/components, routings/operations and work-order material/operation snapshots. `createWorkOrderWithin` uses Decimal arithmetic, validates active same-company master data and snapshots the BOM/routing atomically; `releaseWorkOrderWithin` locks the order and blocks release on material shortage. `work-orders`, `work-order` and `new-work-order` use only bounded formal resources in Demo/API. Issue/return/time/completion/MRP remain the next manufacturing slice. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 110 passing tests plus one gated PostgreSQL 16 integration proof. Includes persistent Session restart, CSRF, RBAC, encrypted account lifecycle, setup, atomic action idempotency/replay/expiry, inventory adjustment snapshot conflicts, transfer conservation, bin/lot/serial invariants, manufacturing snapshot/release rules, complete inventory, purchasing and CRM resource/transaction proofs, audit correlation and migration compatibility. |
...
creation/release use the canonical versioned BOM/routing snapshot model in Demo and API
modes. BOM authoring, material issue/return, production reporting, completion/WIP/GL and
MRP run commands are not complete, so `bom` and `mrp` remain Preview.
