...
| Canonical schema (161 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 56 ordered migrations through `drizzle/0055_leave_payroll_sources.sql`, `src/data/schema/` |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 121 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **119 Canonical / 2 Preview**. Preview pages distinguish Sample Data from Canonical Data and lock write-like actions. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 452 passing tests plus 1 expected skip and one gated PostgreSQL 16 integration proof. Coverage includes Session/CSRF/RBAC, idempotency/audit, inventory/warehouse/manufacturing/quality invariants, sales/purchasing/CRM lifecycles, finance/assets/project/service/HR/payroll postings, actor-owned governed leave, calendar scope/sync, leave-to-Payroll replay and migration compatibility. |
...
deduction/encashment boundary. TASK-117 through TASK-135 remain planning records
...
- Done: 115 tasks, including TASK-116
- Blocked: TASK-017 (1)
- Todo: 19 planned tasks (TASK-117–135) across EPIC-054–056. These extend the product
  beyond the current 119 Canonical / 2 Preview boundary; they do not reopen or
