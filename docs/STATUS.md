...
| Setup wizard (language/org/company/admin/AI preview) writes to PGlite | ✅ Working | `web/public/assets/screens-setup-wizard.js` + `ErpSystemData.completeSetup()` → shared `completeDemoSetupWithin`, gated in `app.js` boot(). The language step also offers four browser-local background styles shared by sign-in, setup, workspace and Settings; company-wide defaults and employee overrides remain a future authenticated preference contract. Production setup remains a separate empty-database/zero-user command and does not require a deployment setup token. |
