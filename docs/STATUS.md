...
| Canonical schema (109 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 36 ordered migrations through `drizzle/0035_sad_paper_doll.sql`, `src/data/schema/` |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **82 Canonical / 32 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 295 passing tests (1 skipped) plus one gated PostgreSQL 16 integration proof (also run in CI). Coverage includes Session/CSRF/RBAC, idempotency/audit, inventory/warehouse/manufacturing/quality invariants, sales/purchasing/CRM lifecycles, finance/assets/project/service/HR/payroll postings and migration compatibility. Supplier-contract coverage adds tenant/date/tier/overlap validation, serialized concurrent activation, idempotent audit replay, Viewer denial and rebuildable vendor-performance reads. |
...
| Service worker never caches `/api/*` or `/health` | ✅ Working | `web/public/sw.js` (`CACHE_VERSION` v55) — the Cache API keys purely on URL and ignores cookies, so caching session-scoped responses could serve a stale signed-in state after logout. |
...
before receipt, without moving stock or writing GL. Supplier contracts and derived vendor
performance are now Canonical; purchasing reports and the shared `pur-txn-view` remain Preview.
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 82 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; supplier controls joined in EPIC-034. |
...
- Done: TASK-001…016, TASK-018…070 (69)
- Blocked: TASK-017 (1)
- Todo in the historical task registry: none. The remaining product work is the 32
