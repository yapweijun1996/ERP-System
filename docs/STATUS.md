...
| `VITE_DATA_MODE=api` renders every current Canonical route | ✅ Working | `erp-system-api-adapter.js` calls the authenticated API with no sample fallback. Dashboard, Inventory, Purchasing, CRM, Sales, Finance and Settings all consume canonical resources/session data. **21 of 21 current Canonical routes support both Demo and API mode.** Company switching re-fetches the authenticated tenant scope. |
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 21 current Canonical routes use `ErpSystemData` in API mode with no sample fallback. Settings reads the authenticated session and labels browser-local preferences honestly. Item master, adjustment UI, advanced warehouse and CRM detail/Customer 360 remain Preview until their schema, adapters, permissions, tests and five-language coverage meet the Canonical gate. |
