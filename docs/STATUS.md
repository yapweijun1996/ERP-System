...
| `VITE_DATA_MODE=api` Canonical boundary | ⚠️ 128 declared API routes / 129 Canonical | `staff-calendar` is the sole metadata exception. Public health/root probes passed on 2026-08-13, but a current authenticated full-route API-layout run and the source decision remain TASK-200. |
...
| Platform Superadmin Demo quick setup | ✅ Source/test/live application release | The flag-on hosted Demo path keeps editable defaults and one-click Demo Platform login while preserving realm isolation, password toggles and tenant-only Remember. The authenticated workspace uses the 80vh/100dvh internal-scroll shell; completed tenant control hides provisioning progress and exposes later Company creation only through `+ Create Company`. Commit `a5f1a3b` passed isolated layout/autofill E2E, both builds, typecheck/lint, permission and 129-route desktop/mobile audits, then shipped application-only on 2026-08-13. Live 1255×872 and 390×844 checks had no overflow or console errors. |
