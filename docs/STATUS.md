...
idempotency and audit dispatcher; receipt stock changes use the shared inventory
    ...
    purchase returns, supplier credit/debit notes, price lists, landed cost and vendor
    performance intentionally still show sample data. **A third
...
| Canonical schema (104 tables, multi-tenant `master_fn`/`company_fn`) | ✅ Working | 33 ordered migrations through `drizzle/0032_backfill_cash_account.sql`, `src/data/schema/` |
...
| Purchasing chain: requisition/RFQ/quote → PO → goods receipt → supplier invoice → return/credit/debit, end-to-end incl. screens | ✅ Canonical Demo/API data and writes | Requisitions, RFQs, invited supplier quotations and the PO/receipt/invoice chain use bounded formal resources in both modes. RFQ award creates one linked draft PO without stock/GL effects. Receipt uses `receiveStockWithin`; supplier-invoice posting creates balanced GL. A receipt/invoice-linked purchase return atomically issues stock and creates an immutable supplier credit. Commercial supplier debit notes instead post Dr AP / Cr Purchase Variance / Cr Input Tax without stock movement. Both adjustments feed one remaining-payable calculation used by Payment Voucher, preventing AP overpayment. Price lists, landed cost and vendor performance remain Preview. |
...
| Route production metadata and Preview contract | ✅ Working | `SCREEN_META` covers all 114 routes with module, Canonical/Preview maturity, data source, supported modes, active section, permission and fixture. Current baseline: **75 Canonical / 39 Preview**. Preview pages show `Preview · Sample Data` consistently and their write-like actions are disabled with an explanation. |
...
| Unit/API tests: domain chains, rollback, GL balance, auth security and API contracts | ✅ Working | `npm test`, 280 passing tests (1 skipped) plus one gated PostgreSQL 16 integration proof (also run in CI). Coverage includes Session/CSRF/RBAC, idempotency/audit, inventory/warehouse/manufacturing/quality invariants, sales/purchasing/CRM lifecycles, finance/assets/project/service/HR/payroll postings and migration compatibility. Supplier debit coverage adds amount caps, replay/audit, balanced GL, no-stock and net-payment proofs. |
...
| Service worker never caches `/api/*` or `/health` | ✅ Working | `web/public/sw.js` (`CACHE_VERSION` v48) — the Cache API keys purely on URL and ignores cookies, so caching session-scoped responses could serve a stale signed-in state after logout. |

## Canonical and Preview route boundary

114 routes are registered in the live `SCREENS` registry. `SCREEN_META` is now the source
of truth for production maturity at route level: **75 routes are Canonical and 39 are
...
**Purchasing remains partially converted, with sourcing, supplier returns and commercial
debits now Canonical (TASK-064…066, 2026-07-22).** Requisitions, RFQs, invited supplier quotations,
...
outstanding calculation. Supplier price lists, landed cost, vendor performance,
...
| --- | --- |
| `VITE_DATA_MODE=api` renders every current Canonical screen with real data | **Complete for the present Canonical boundary.** All 75 current Canonical routes use `ErpSystemData` in API mode with no sample fallback, confirmed via `npm run audit:screens`; supplier debit notes joined in EPIC-030. |
