...
首次使用或七天后失效，并强制首次登录改密。正式环境提供仅限 Company Owner 的员工工作区
入口；目标按公司范围限制，进入、返回或退出都会记录审计，且不会暴露员工密码。
...
fixture 间接提供 Company Owner 与 Viewer。`Avery Tan · Company Owner` 同时属于 SG/MY，
固定显示在人物切换器第一位，可进入有明确租户权限的设置与模块启用控制；其他人物会显示其真实
岗位，不会再全部误标成 Viewer。旧 IndexedDB 确认升级后会保留既有业务数据并补齐身份。

正式 PostgreSQL 严禁 Demo seed。首次空库 setup 只建立组织、首家公司和
Company Owner，之后依序完成公司/税务、财年/科目、仓库、模块、角色、员工账号、
