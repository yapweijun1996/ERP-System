...
try {
  ...
  for (const width of [1280, 375]) {
    ...
    await page.locator('#nePassword').fill('Synthetic-fixture-password-123!');
    ...
    const createdNavigation = page.waitForEvent('framenavigated', {
      predicate: frame => frame === page.mainFrame(), timeout: 60000,
    });
    await page.locator('#neNext').click();
    await page.waitForFunction(() => (typeof DB !== 'undefined' && DB.user?.email === 'immediate.staff@example.test')
      || document.querySelector('[data-staff-onboarding-error]:not([hidden])')?.textContent.trim()
      || document.querySelector('.toast .td'), null, { timeout: 60000 }).catch(async error => {
      const state = await page.evaluate(() => ({ url: location.href, email: typeof DB !== 'undefined' ? DB.user?.email : null,
        selectedEmail: localStorage.getItem('aria-active-user-email'),
        text: document.querySelector('#viewRoot')?.innerText.slice(-1800),
        busy: document.querySelector('#neNext')?.disabled }));
      throw new Error(`${error.message}: ${JSON.stringify(state)}`);
    });
    await createdNavigation;
    await page.waitForFunction(() => typeof DB !== 'undefined' && DB.user?.email === 'immediate.staff@example.test'
      && !document.body.classList.contains('auth-locked'), null, { timeout: 60000 });
    const creationError = await page.evaluate(() => document.querySelector('[data-staff-onboarding-error]:not([hidden])')?.textContent
      || document.querySelector('.toast .td')?.closest('.toast')?.textContent || '');
    if (creationError?.trim()) throw new Error(`Staff creation failed: ${creationError.trim()}`);
    const account = await page.evaluate(async () => {
      const rows = await ErpSystemData.db.query(
        'select account_state,password_change_required,initial_password_expires_at from app_user where master_fn=$1 and email=$2',
    ...
    await page.locator('#loginPassword').fill('Synthetic-fixture-password-123!');
